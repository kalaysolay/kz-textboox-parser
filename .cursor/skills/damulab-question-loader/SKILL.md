...
Этот скилл грузит **только текст** из `05`. Файлы из `06-illustrations.json` /
`illustrations/` на сайт не заливает — для этого будет отдельный asset-import агент
(контракт уже в [contracts.md](../damulab-question-pipeline/contracts.md)). Пока ручки
ассетов в банке нет, прогон с картинками остаётся локально в статусе
...
- Не импортировать SVG/PNG из `illustrations/` через этот скилл.
